...
> **Статус на 22.09.2026:** проверены CAN-передача и stop/start, обычные ioctl и рассылка событий двум читателям. Атрибуты sysfs state/temp_status реализованы; подтверждены начальные stopped/normal и изменение после команд до running/overheat и stopped/overheat. Граничные сценарии ещё требуют проверки. Procfs реализован: проверены сводка, подключение/закрытие читателя и удаление интерфейсов при выгрузке.
...
## Посмотреть состояние или слушать события

| Задача | Команда |
| --- | --- |
| Сразу получить все текущие поля | `sudo ./tools/boardctl status` |
| Прочитать состояние передачи | `cat /sys/class/virtual_board/virtual_board/state` |
| Прочитать температурный статус | `cat /sys/class/virtual_board/virtual_board/temp_status` |
| Получить сводку состояния и клиентов | `cat /proc/virtual_board` |
| Ждать новые события | `sudo cat /dev/virtual_board` |

`cat /dev/virtual_board` не выдаёт начальный снимок: новый читатель получает пустую очередь и ждёт события. После вывода события он продолжает слушать; выход — Ctrl+C. Чтение sysfs и `boardctl status` сразу возвращают текущее состояние, уже существующее в памяти платы, и завершаются. Это поведение задано обработчиками нашего модуля. [Схема вызовов и назначение интерфейсов](docs/INTERFACES.md#почему-один-cat-ждёт-а-другой-сразу-отвечает).
...
При выключении vcan0 плата сохраняет running и продолжает попытки. Читатели получают can_error с отрицательным кодом; повтор одного кода подавляется до успешной отправки. После up того же интерфейса передача восстанавливается без start — проверено. candump может завершиться с Network is down; его нужно запустить снова. После удаления интерфейса автоматическое переподключение не предусмотрено: требуется подготовка интерфейса и перезагрузка модуля; этот сценарий ещё не проверен.
